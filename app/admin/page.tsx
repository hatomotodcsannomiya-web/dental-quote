...
function SortableDepositRow({ item, editingId, editName, setEditName, setEditingId, updateItem, toggleActive, deleteItem }: {
  item: DepositItem;
  editingId: number | null;
  editName: string;
  setEditName: (v: string) => void;
  ...
  toggleActive: (item: DepositItem) => void;
...
function DepositItemsTab() {
  ...
  async function load() {
    const res = await fetch("/api/deposit-items");
  ...
  async function updateItem(id: number) {
    ...
    await fetch(`/api/deposit-items/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: editName }),
    });
  ...
  async function toggleActive(item: DepositItem) {
    await fetch(`/api/deposit-items/${item.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  async function deleteItem(id: number) {
    if (!confirm("この預かり品項目を削除しますか？")) return;
    await fetch(`/api/deposit-items/${id}`, { method: "DELETE" });
    load();
  }
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    setItems(reordered);
    await fetch("/api/deposit-items/reorder", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(reordered.map((m, i) => ({ id: m.id, sortOrder: i }))),
    });
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-5">
      <h2 className="text-base font-semibold text-gray-800 mb-1">預かり品管理</h2>
      <p className="text-xs text-gray-400 mb-4">技工指示書作成時に選択できる預かり品の項目を管理します。</p>
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext items={items.map((m) => m.id)} strategy={verticalListSortingStrategy}>
          <div className="space-y-2">
            {items.length === 0 && <p className="text-sm text-gray-400">預かり品項目が登録されていません</p>}
            {items.map((item) => (
              <SortableDepositRow
                key={item.id}
                item={item}
